...
descride '投稿画面(todolists_new_path)のテスト' do
  before do
  ...
  context '投稿処理のテスト' do
    it '投稿後のリダイレクト先は正しいか' do
      fill_in 'list[title]', with: Faker::Lorem(number:5)
      fill_in 'list[body]', with: Faker::Lorem(number:20)
      click_button '投稿'
...
describe '詳細画面(todolist_show_path)のテスト' do
  before do
